...
def check_software_moveable(source_path: str, software_name: str) -> Dict:
    ...
    # Check 1: UWP/MSIX app detection
    ...
    # Check 4: System directory files (should be empty after fix)
    ...
    for attempt in range(retries):
        try:
            ...
            if attempt < retries - 1:
                time.sleep(0.5)
    _log(log_cb, f"跳过锁定文件: {os.path.basename(source)} ({last_err})")
    return False
...
def _create_link(link_path: str, target_path: str, link_mode: str, log_cb: LogFn) -> None:
    ...
    if link_mode == "junction":
        ...
        try:
            ...
            _log(log_cb, "Junction created")
            return
        except subprocess.CalledProcessError as exc:
            raise RuntimeError(f"Failed to create junction: {exc.stderr or exc}")
    ...
    def run(self) -> Dict:
        try:
            ...
            if os.path.exists(self.source_path):
                backup_path = f"{self.source_path}_backup"
                os.rename(self.source_path, backup_path)
                _log(self.log_cb, f"Source still existed, renamed to: {backup_path}")
                self.source_path = backup_path

            _log(self.log_cb, "Creating link...")
            _create_link(self.source_path.replace("_backup", ""), self.target_path, self.link_mode, self.log_cb)

            if self.source_path.endswith("_backup") and os.path.exists(self.source_path):
                try:
                    shutil.rmtree(self.source_path, ignore_errors=True)
                    _log(self.log_cb, "Backup removed")
                except Exception:
                    _log(self.log_cb, f"Backup not removed: {self.source_path}")

            return {
                "ok": True,
                "source_path": self.source_path.replace("_backup", ""),
