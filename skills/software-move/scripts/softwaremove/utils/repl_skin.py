"""REPL Skin for softwaremove CLI."""
...
class ReplSkin:
    ...
    def __init__(self, software: str, version: str = "1.0.0",
        ...
        Args:
            ...
            history_file: Path for persistent command history.
                         Defaults to ~/.softwaremove/history
        ...
        if history_file is None:
            from pathlib import Path
            hist_dir = Path.home() / ".softwaremove"
    ...
    def _detect_color_support(self) -> bool:
        ...
        if os.environ.get("NO_COLOR"):
            return False
        if os.environ.get("SOFTWAREMOVE_NO_COLOR"):
            return False
    ...
    def print_banner(self):
        ...
        brand = self._c(_CYAN + _BOLD, "softwaremove")
        name = self._c(self.accent + _BOLD, self.display_name)
        title = f" {icon}  {brand}" + (f" {self._c(_DARK_GRAY, '·')} {name}" if name.lower() != "softwaremove" else "")
